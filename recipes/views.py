...
def index(request):
    ...
    print("tags", tags)
    template_name = (
        "indexAuth.html" if request.user.is_authenticated else "indexNotAuth.html"
    )
...
def recipe_view(request, recipe_id):
    ...
    template_name = (
        "singlePage.html" if request.user.is_authenticated else "singlePageNotAuth.html"
    )
...
def subscriptions(request):
    author_list = User.objects.prefetch_related("recipe_author").filter(
...
def download(request):
    ...
    """
    ingredient_list = (
        Recipe.objects.prefetch_related("ingredient", "recipe_amount")
