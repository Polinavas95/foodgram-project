...
class TestFavorite(TestCase):
    def setUp(self):
        ...
        self.user1 = User.objects.create_user(
            username='user1', email='user1@.com', password='1234567')
        self.user2 = User.objects.create_user(
            username='user2', email='user2@.com', password='123456')
    ...
    def test_follow_favorite(self):
        response = self.client.post(
            reverse('favorites'), data={'id': 1})
        assert response.status_code == 403, \
            'Незалогиненный пользователь не может добавить рецепт в избранное'
        self.client.login(username='user1', password='1234567')
        response = self.client.post(reverse('favorites'), data={'id': 2})
        assert response.status_code == 200, \
            'Залогиненный пользователь может добавить чужой рецепт в избранное'
        assert response.data == {'success': True}, \
            'При успешном добавлении возвращается "True"'
        response = self.client.post(reverse('favorites'), data={'id': 2})
        assert response.status_code == 200, \
            'Залогиненный пользователь может добавить чужой рецепт в избранное'
        assert response.data == {'success': False}, \
            'Пользователь не может повторно добавить рецепт в избранное'
        response = self.client.post(reverse('favorites'), data={'id': 1})
        assert response.data == {'success': False}, \
    ...
    def test_unfollow_favorite(self):
        ...
        self.client.post(reverse('favorites'), data={'id': 2})
