...
class TestUser(TestCase):
    ...
    def test_profile_page(self):
        ...
        ), "Пользователь не может перейти на страницу профайла другого пользователя"
        assert (
            response.context["username"] == self.user.username
    ...
    def test_auth_client_create_recipe_post(self):
        ...
        assert "text1" in [
            recipe.text for recipe in recipes
        ], "Текс созданного рецепта не совпадает с тем что в БД (POST-запрос)"
        assert (
            recipes.count() == 1
        ), "Рецепт не был создан зарегестрированным пользователем (POST-запрос)"
    ...
    def test_auth_client_create_recipe_get(self):
        ...
        assert (
            recipes.count() == 0
        ), "Рецепт был создан зарегестрированным пользователем (GET-запрос)"
    ...
    def test_non_auth_client_create_recipe_post(self):
        ...
        assert (
            recipes.count() == 0
        ), "Рецепт был создан незарегестрированным пользователем (POST-запрос)"
    ...
    def test_nonauth_client_create_recipe_get(self):
        ...
        assert (
            recipes.count() == 0
        ), "Рецепт был создан незарегестрированным пользователем (GET-запрос)"
